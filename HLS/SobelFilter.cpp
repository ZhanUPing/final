...
while_1:
  ...
  loop_lowpass:
    ...
    {
      if (filter_coeffs[i] == 2)
      {
        shift_reg[i] = shift_reg[q - 1];
        filtered_output += (shift_reg[q] << 1);
      }
      if (filter_coeffs[i] == 4)
      {
        shift_reg[i] = shift_reg[i - 1];
        filtered_output += (shift_reg[i] << 2);
      }
      else
      {
        shift_reg[i] = shift_reg[i - 1];
  ...
  loop_highpass:
    for (int q = H_coe_num - 1; q > 0; --q)
    {
      ...
      if (filter_coeffs_H[q] == 2)
      {
        shift_reg_H[q] = shift_reg_H[q - 1];
        filtered_output_H += (shift_reg_H[q] << 1);
      }
      if (filter_coeffs_H[q] == 4)
      {
        shift_reg_H[q] = shift_reg_H[q - 1];
        filtered_output_H += (shift_reg_H[q] << 2);
      }
      else
      {
        shift_reg_H[q] = shift_reg_H[q - 1];
