...
define_hls_config SobelFilter FLAT_UNROLL_ALL_FAST --flatten_arrays=all -post_elab_tcl {
    unroll_loops [find -loop "loop*" ]
    constrain_latency -max_lat $HLS::ACHIEVABLE [find -loop "while_1"]
}
define_hls_config SobelFilter FLAT_UNROLL_ALL --flatten_arrays=all -post_elab_tcl {
    unroll_loops [find -loop "loop*" ]
...
### 5.4 HLS config "FLAT_UNROLL_ALL_FAST"
define_sim_config V_FLAT_UNROLL_ALL_FAST "SobelFilter RTL_V FLAT_UNROLL_ALL_FAST" -argv "$IN_FILE_NAME $OUT_FILE_NAME"
### 5.5 HLS config "FLAT_UNROLL_ALL"
define_sim_config V_FLAT_UNROLL_ALL "SobelFilter RTL_V FLAT_UNROLL_ALL" -argv "$IN_FILE_NAME $OUT_FILE_NAME"
